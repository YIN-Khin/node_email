...
async function testExpirationSystem() {
  try {
    ...
    const expiringToday = await NotificationService.createNotification({
      ...
      message:
        'ផលិតផល "Test Product 2" ផុតកំណត់ថ្ងៃនេះ! សូមពិនិត្យមុនពេលប្រើប្រាស់។',
      referenceId: 3,
    });
    ...
    const expired = await NotificationService.createNotification({
      ...
      message:
        'ផលិតផល "Test Product 3" បានផុតកំណត់ 2 ថ្ងៃហើយ! សូមយកចេញពីស្តុក។',
      referenceId: 4,
    });
    ...
    const allNotifications =
      await NotificationService.getRecentNotifications(10);
    ...
    expirationNotifications.forEach((notification) => {
      ...
      if (notification.Product) {
        console.log(
          `    Product: ${notification.Product.name} (expires: ${notification.Product.expire_date})`,
        );
      }
    });
    ...
    console.log(
      "   3. Create sample notifications: POST http://localhost:3001/api/notifications/samples/create",
    );
